...
function ExamNode(_props: NodeProps) {
  const {
    nodes,
    examMode,
    examCompleted,
  ...
  const pct = Math.min(100, Math.round((masteredCount / BOSS_UNLOCK_THRESHOLD) * 100));

  /* ExamMode はフルスクリーンオーバーレイなので、ノード自体は残す */
  if (examMode) return <div style={{ width: 90, height: 110 }} />;
    ...
    >
      ...
      >
        {galaxyCompleted
          ...
          : isUnlocked
          ? "✨ 過去問 解放！"
          : `あと ${BOSS_UNLOCK_THRESHOLD - masteredCount} 個で解放`}
      </div>
      ...
      <button
        ...
        style={{
          ...
          boxShadow: isUnlocked
            ? "0 0 20px rgba(251,191,36,0.45), 0 0 44px rgba(251,191,36,0.2), inset 0 -4px 12px rgba(0,0,0,0.4)"
